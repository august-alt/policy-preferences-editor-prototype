...
std::unique_ptr<PreferencesModel> NetworkModelBuilder::schemaToModel(std::unique_ptr<NetworkOptions> &network)
{
    ...
    for (const auto& networkoptionsschema : network->VPN())
    {
        ...
        for (const auto& properties: networkoptionsschema.Properties())
        {
            ...
            vpn.setProperty(VpnItem::propertyToString(VpnItem::ACTION), state);
            ...
            vpn.setProperty<std::string>(VpnItem::propertyToString(VpnItem::IP_ADDRESS), properties.ipAddress());
            ...
            vpn.setProperty<std::string>(VpnItem::propertyToString(VpnItem::REDIAL_PAUSE), getOptionalPropertyData(properties.redialPause()));
    ...
    for (const auto& networkoptionsschema : network->DUN())
    {
        ...
        for (const auto& properties: networkoptionsschema.Properties())
        {
            ...
            auto action = properties.action();
            auto state = getActionCheckboxState(action.present() ? action->c_str() : "");
            dun.setProperty(DialUpItem::propertyToString(DialUpItem::ACTION), state);
