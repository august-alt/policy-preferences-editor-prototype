...
void SharesWidget::submit()
{
    if (mapper && validate())
    {
        ...
        {
            m_item->setProperty(mvvm_folders::SharesItem::ACCESS_BASED_ENUMERATION, "NO_CHANGE");
        }

        if (ui->enableAccess->isChecked())
        {
            m_item->setProperty(mvvm_folders::SharesItem::ACCESS_BASED_ENUMERATION, "ENABLE");
        }

        if (ui->disableAccess->isChecked())
        {
            m_item->setProperty(mvvm_folders::SharesItem::ACCESS_BASED_ENUMERATION, "DISABLE");
        }

        if (ui->noChangeUsers->isChecked())
        {
            m_item->setProperty(mvvm_folders::SharesItem::LIMIT_USERS, "NO_CHANGE");
        }

        if (ui->maximumAllowedUsers->isChecked())
        {
            m_item->setProperty(mvvm_folders::SharesItem::LIMIT_USERS, "MAX_ALLOWED");
        }

        if (ui->allowThisNumberOfUsers->isChecked())
        {
            m_item->setProperty(mvvm_folders::SharesItem::LIMIT_USERS, "SET_LIMIT");
        }
