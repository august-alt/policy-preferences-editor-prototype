...
void PreferencesDialogTest::show()
{
    ...
    if (file.good()) {
       ...
       auto modelBuilder = std::make_unique<preferences_editor::ModelBuilder>();
       auto model = modelBuilder->schemaToModel(shortcuts);

       auto selectionModel = std::make_unique<QItemSelectionModel>(model.get());

       auto widget = new preferences_editor::ShortcutsWidget(*model, *selectionModel);
       auto preferencesDialog = std::make_unique<preferences_editor::PreferencesDialog>(widget, *model, *selectionModel);
